...
class Ship:
    ...
    def update(self):
        ...
        self.forward=angle_to_vector(self.angle)       
        if(self.thrust):
            self.vel[0]+=.1*self.forward[0]
            self.vel[1]+=.1*self.forward[1]     
    ...
    def shoot(self):
        global a_missile
        a_missile = Sprite([self.pos[0]+self.forward[0]*45,self.pos[1]+self.forward[1]*45],[self.vel[0]+(6*self.forward[0]),self.vel[1]+(6*self.forward[1])],self.angle,0,missile_image,missile_info,missile_sound)
